...
def load_test_samples(path: str):
    """
    Supports:
    1) raw labeled list: text/company/ticker/cause/effect/event_category/impact_direction
    2) messages format list (eval.json style)
    """
...
def write_markdown(out_path, baseline_metrics, finetuned_metrics, test_file):
    lines = []
    lines.append("# Test Set Baseline vs Finetuned")
    lines.append("")
    ...
    if not finetuned_metrics["top_category_mismatches"]:
        ...
        for m in finetuned_metrics["top_category_mismatches"][:8]:
            lines.append(f"- `{m['ground_truth']} -> {m['predicted']}`: {m['count']}")
...
def main():
    ...
    parser.add_argument("--test", default="data/test_set_batch_no_premarket.json")
    parser.add_argument("--base-model", default="Qwen/Qwen2.5-7B-Instruct")
    parser.add_argument("--adapter", default="./qwen2.5-7b-finetuned-3may26")
    parser.add_argument("--outdir", default="runs/2026-05-03_testset_eval")
    ...
    samples = load_test_samples(args.test)
    ...
    baseline_path = os.path.join(args.outdir, "baseline_results_qwen7b_testset.json")
    ...
    finetuned_path = os.path.join(args.outdir, "finetuned_results_qwen7b_testset.json")
    ...
    summary_json = os.path.join(args.outdir, "comparison_testset_exact.json")
    json.dump(
        {"baseline": baseline_metrics, "finetuned": finetuned_metrics},
        open(summary_json, "w", encoding="utf-8"),
        indent=2,
    )
    summary_md = os.path.join(args.outdir, "comparison_testset_summary.md")
    write_markdown(summary_md, baseline_metrics, finetuned_metrics, args.test)
